...
public class ScoreTracker {
    ...
    public void addScore(Score s) {
        ...
        if (minAttempts > s.getNumAttempts()) {
            this.minAttempts = s.getNumAttempts();


        }


        if(highPointsEarned < s.getPointsEarned()) {
            this.highPointsEarned = s.getPointsEarned();
        }
    ...
    private int findHighestPointsEarned() {
        int max = scores.get(0).getPointsEarned();
        if(!scores.isEmpty()){
             for(Score s: scores) {
                 if(s.getPointsEarned() > max) {
                     max = s.getPointsEarned();
    ...
    private int findMinAttemptsTaken() {
        ...
        if(!scores.isEmpty()){
            for(Score s: scores) {
                if(s.getNumAttempts() < min) {
                    min = s.getNumAttempts();
